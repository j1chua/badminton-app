        ...
        for idx, row in df.iterrows():
            ...
            for c in blocks:
                try:
                    ...
                    # Look in the specific column for the court header
                    court = "Court ?"
...
if sch is None or sch.empty:
    ...
    with tab1:
        ...
        for v in st.session_state.db.values():
            if v['t1'] in stats:
                stats[v['t1']]["Sets Won"] += v['w1']; stats[v['t1']]["Total Pts"] += v['p1']
            if v['t2'] in stats:
                stats[v['t2']]["Sets Won"] += v['w2']; stats[v['t2']]["Total Pts"] += v['p2']
    ...
    with tab2:
        ...
        if rows:
            ...
            # SORTING: Sort by Court name, then by Time
            sched_df = sched_df.sort_values(by=["Court", "Time"])
