...
<style>
    ...
    .forfeit { color: #d32f2f; font-weight: bold; font-style: italic; }
...
if sch is None or sch.empty:
    st.warning("Please ensure 'SMASH 2026 - Score Tracker.csv' is in the folder.")
else:
    ...
    tab1, tab2, tab_finals, tab_admin = st.tabs(["📊 Standings", "📅 Schedule", "🏆 Finals", "⚙️ Admin"])
    ...
    with tab2:
        ...
        for _, r in sch[sch["Day"] == day_pick].iterrows():
            if q in r['T1'].lower() or q in r['T2'].lower() or q in r['P1'].lower() or q in r['P2'].lower():
                ...
                rows.append({"Time": r["T"], "Court": r["Court"], "Bracket": EMOJIS.get(r['L'], "🏸"), "Match": f"{r['P1']} vs {r['P2']}", "Set 1": s1, "Set 2": s2})
        st.write(pd.DataFrame(rows).to_html(escape=False, index=False, classes="m-table"), unsafe_allow_html=True)

    # --- TAB 3: FINALS ---
    with tab_finals:
        sel_b = st.selectbox("Switch Bracket View:", all_brackets, key="view_br")
        ...
        if st.text_input("Admin Password", type="password") == ADMIN_PW:
            ...
            def admin_match(label, subkey, p1_def, p2_def):
                ...
                with c4:
                    ...
                    s3a = st.number_input("S3a", 0, 31, value=d.get('s3a', 0), key=f"s3a_{k}_{v}", label_visibility="collapsed") if has_s3 else 0
                    s3b = st.number_input("S3b", 0, 31, value=d.get('s3b', 0), key=f"s3b_{k}_{v}", label_visibility="collapsed") if has_s3 else 0
                
                w1 = (s1a > s1b) + (s2a > s2b) + (1 if has_s3 and s3a > s3b else 0)
                w2 = (s1b > s1a) + (s2b > s2a) + (1 if has_s3 and s3b > s3a else 0)
