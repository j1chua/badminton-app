...
with t3:
    ...
    # INTEGRATED PLAYOFF BRACKETS FROM CSV
                ...
                if any(x in label for x in ["SEMIS", "FINALS"]):
                    ...
                    if "|" in tn1:
                        ...
                        playoff_rows.append({
                            "BRACKET": label, 
                            "TEAM 1": tn1, 
                            "SCORE 1": sc1 if sc1 else "0", 
                            "TEAM 2": tn2, 
                            "SCORE 2": sc2 if sc2 else "0"
                        })
    if playoff_rows:
...
with t4:
    ...
    for v in ["PURPLE", "RED", "YELLOW", "BLACK", "GREEN", "WHITE"]:
        ...
        st.markdown(f'<div style="background-color:{COLOR_MAP.get(v, "#333")}; color:white; padding:10px; border-radius:5px; margin-top:20px; font-weight:bold;">{EMOJIS.get(v, "")} {v} BRACKET</div>', unsafe_allow_html=True)
        ...
        if winners:
            ...
            for i, team in enumerate(winners):
                with cols[i]:
                    ...
                    st.markdown(f'<div style="text-align:center; padding:10px; border:1px solid #ddd; border-radius:5px;">{rank_html}<br>{team}</div>', unsafe_allow_html=True)
...
with t5:
    ...
    if st.text_input("Password", type="password") == ADMIN_PW:
        ...
        d = finals_data.setdefault(v, {
            "s1":{'t1':'','t2':'','s1a':0,'s1b':0,'s2a':0,'s2b':0,'s3a':0,'s3b':0,'use_s3':False,'sw1':0,'sw2':0},
        ...
        for m_id, label in [("s1", "Semi 1"), ("s2", "Semi 2"), ("f", "Finals")]:
            ...
            w1_temp = (1 if md['s1a'] > md['s1b'] else 0) + (1 if md['s2a'] > md['s2b'] else 0)
            w2_temp = (1 if md['s1b'] > md['s1a'] else 0) + (1 if md['s2b'] > md['s2a'] else 0)
            ...
            if st.button(f"Save {label}", key=f"save_{m_id}_{v}"):
                md['sw1'] = w1_temp + (1 if md['use_s3'] and md['s3a'] > md['s3b'] else 0)
                md['sw2'] = w2_temp + (1 if md['use_s3'] and md['s3b'] > md['s3a'] else 0)
