...
if sch is None or sch.empty:
    st.warning("Please ensure the CSV file is uploaded and formatted correctly.")
else:
    ...
    # --- TAB 1: STANDINGS ---
    with tab1:
        st.info("🕒 **Current Leaderboard** (Based on Match Results)")
    ...
    # --- TAB 2: DAY 1 ---
    with tab2:
        q = st.text_input("🔍 Search Team Name (Day 1)", key="q1").lower()
        rows = []
    ...
    with tab3:
        ...
        st.success("🔥 Day 2 matches are ongoing. Brackets will update as scores are recorded.")

    # --- TAB 4: FINALS (VIEW ONLY) ---
    with tab_view:
        ...
        render_view("🏆 Championship Final", "fin")
    ...
    with tab_admin:
        ...
        if pw == ADMIN_PW:
            ...
            def admin_match(label, k, p1_def, p2_def):
                ...
                m_state = {"t1":t1, "t2":t2, "s1a":s1a, "s1b":s1b, "s2a":s2a, "s2b":s2b, "s3a":s3a, "s3b":s3b, "has_s3": has_s3, "winner": winner}
                with c5:
                    if st.button("💾 Save", key=f"save_{k}_{v}", use_container_width=True, type="primary"):
                        save_single_match(k, m_state); st.toast(f"{label} Saved!"); st.rerun()
                with c6:
                    if st.button("🔄 Reset", key=f"reset_{k}_{v}", use_container_width=True):
                        reset_s = {"t1":p1_def, "t2":p2_def, "s1a":0, "s1b":0, "s2a":0, "s2b":0, "s3a":0, "s3b":0, "has_s3": False, "winner":"TBD"}
                        save_single_match(k, reset_s); st.session_state.reset_n += 1; st.rerun()
                st.divider()
