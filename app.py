...
<style>
    ...
    .bracket-header { background-color: #000; color: #fff; padding: 12px; border-radius: 5px; text-align: center; margin-bottom: 20px; font-weight: bold; }
...
if sch is None or sch.empty:
    ...
    with tabs[0]:
        ...
        for color in all_brackets:
            ...
            sdf["Team"] = sdf["Team"].str.replace("|", " AND ")
    ...
    # --- TAB 3: DAY 2 ---
    with tabs[2]:
        st.info("Day 2 Schedule in Progress...")

    # --- TAB 4: FINALS (VIEW) ---
    with tabs[3]:
        sel_b = st.radio("Select Bracket:", all_brackets, horizontal=True)
    ...
    with tabs[4]:
        if st.text_input("Password", type="password") == ADMIN_PW:
            adm_b = st.selectbox("Manage Bracket:", all_brackets)
            b_teams = sorted([t.replace("|", " AND ") for t, c in clrs.items() if c == adm_b])
            st.markdown(f'<div class="bracket-header">⚙️ {adm_b} ADMIN</div>', unsafe_allow_html=True)
            def adm_m(label, subk, p1_d, p2_d):
                ...
                with c4:
                    ...
                    if has_s3:
                        sc3 = st.columns(2)
                        s3a, s3b = sc3[0].number_input("S3a", 0, 31, value=d.get('s3a', 0), key=f"s3a_{k}_{v}", label_visibility="collapsed"), sc3[1].number_input("S3b", 0, 31, value=d.get('s3b', 0), key=f"s3b_{k}_{v}", label_visibility="collapsed")
                
                wins1 = (s1a>s1b) + (s2a>s2b) + (1 if has_s3 and s3a>s3b else 0)
                wins2 = (s1b>s1a) + (s2b>s2a) + (1 if has_s3 and s3b>s3a else 0)
                win = t1 if wins1 >= 2 else (t2 if wins2 >= 2 else "TBD")
