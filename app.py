...
if sch is None or sch.empty:
    ...
    with tab4:
        ...
        def render_compact_match(match_label, key_suffix):
            ...
            # Layout matching the requested compact line
            c1, c2, c3, c4, c5 = st.columns([3, 1, 1, 1, 1.5])
            
            with c1:
                st.selectbox(f"T1_{key_suffix}", black_teams, key=f"t1_{key_suffix}", label_visibility="collapsed")
                st.selectbox(f"T2_{key_suffix}", black_teams, key=f"t2_{key_suffix}", label_visibility="collapsed")
            
            with c2:
                s1a = st.number_input(f"s1a_{key_suffix}", 0, 30, key=f"s1a_{key_suffix}", label_visibility="collapsed")
                s1b = st.number_input(f"s1b_{key_suffix}", 0, 30, key=f"s1b_{key_suffix}", label_visibility="collapsed")
            with c3:
                s2a = st.number_input(f"s2a_{key_suffix}", 0, 30, key=f"s2a_{key_suffix}", label_visibility="collapsed")
                s2b = st.number_input(f"s2b_{key_suffix}", 0, 30, key=f"s2b_{key_suffix}", label_visibility="collapsed")
            with c4:
                s3a = st.number_input(f"s3a_{key_suffix}", 0, 30, key=f"s3a_{key_suffix}", label_visibility="collapsed")
                s3b = st.number_input(f"s3b_{key_suffix}", 0, 30, key=f"s3b_{key_suffix}", label_visibility="collapsed")
            ...
            with c5:
                ...
                st.markdown(f"<div class='sets-won-result'>{sw1} Set(s) Won</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='sets-won-result'>{sw2} Set(s) Won</div>", unsafe_allow_html=True)
            st.divider()
