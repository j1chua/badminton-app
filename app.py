...
if sch is None: st.error("CSV Missing")
    ...
    with t1:
        ...
        for c in sorted(df_r["Bracket"].unique()):
            ...
            disp_df = df_r[df_r["Bracket"]==c].sort_values(["Sets Won","Total Pts"], ascending=False).drop(columns=["Bracket"])
            # Added hide_index=True
    ...
    with t2:
        ...
        if L: 
            # Added hide_index=True and switched to dataframe for better control
            st.dataframe(pd.DataFrame(L), hide_index=True, use_container_width=True)
