...
if sch is None: st.error("CSV Missing")
    ...
    with t3:
        ...
        if pw == "pogisiJordan":
            ...
            sel = st.selectbox("Select Match", m_filt)
            if sel:
    ...
    with t2:
        ...
        L = [{"Time":r["T"], "Brk":r["L"], "Match":f"{r['T1']} vs {r['T2']}", "Result":st.session_state.db.get(r["ID"],{}).get("sc","--")} for _,r in sch[sch["Day"] == v_day].iterrows() if q in r['T1'].lower() or q in r['T2'].lower()]
        if L: st.table(L)
