...
# Persistence for Finals (New for Admin/Finals tabs)
...
# 2. Data Loading Logic (EXACTLY as per your file)
@st.cache_data
def load_data():
...
if sch is None or sch.empty:
    ...
    # --- TAB 1: STANDINGS (ORIGINAL LOGIC) ---
    with tab1:
    ...
    # --- TAB 2: DAY 1 (ORIGINAL LOGIC - SEPARATE SEARCH) ---
    with tab2:
        q1 = st.text_input("🔍 Search Day 1 Team", key="search_day1").lower()
        rows1 = []
    ...
    # --- TAB 3: DAY 2 (ORIGINAL LOGIC - SEPARATE SEARCH) ---
    with tab3:
        q2 = st.text_input("🔍 Search Day 2 Team", key="search_day2").lower()
        rows2 = []
    ...
    # --- TAB 4: FINALS (WITH BRACKET TOGGLE) ---
    with tab_view:
        sel_v = st.radio("View Bracket Finals:", all_brackets, horizontal=True, key="view_sel")
    ...
    with tab_admin:
        if st.text_input("Admin Password", type="password") == ADMIN_PW:
            sel_a = st.selectbox("Select Bracket to Edit:", all_brackets)
