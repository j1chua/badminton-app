...
def load_tournament_data():
    ...
    for c in configs:
        ...
        for i in range(r_start, r_end):
            try:
                t1, t2 = str(df.iloc[i, idx_t1]).strip(), str(df.iloc[i, idx_t2]).strip()
...
if sch_df is None:
    ...
    with tab3:
        st.header("Admin: Record or Reset Result")
        ...
        with btn_col2:
            ...
            if st.button("🗑️ Reset This Match", use_container_width=True, disabled=not confirm_reset):
                if sel in st.session_state.db:
                    del st.session_state.db[sel]
                    st.warning(f"Cleared scores for {d['T1']} vs {d['T2']}")
                else:
                    st.info("No scores found for this match to reset.")
        ...
        for court in courts:
            with st.expander(f"📍 {court}", expanded=True):
                ...
                for _, row in c_matches.iterrows():
                    ...
                    display_data.append({
                        "Time": row["Time"], "Bracket": row["Color"],
                        "Matchup": f"{row['T1']} vs {row['T2']}", "Result": score
                    })
